...
local offsetWidth = 50
local offsetHeight = 20
...
function SpawnParticle(x, y)
    ...
    if x >= 2 and x <= width - offsetWidth - 1 and y >= 2 and y <= height - offsetHeight - 1 then
        for i = 0, size - 1 do
            if x + i > width then
                break
            end
    ...
    if x >= 2 and x <= width - offsetWidth - 1 and y >= 2 and y <= height - offsetHeight - 1 then
        for i = 0, size - 1 do
            ...
            for j = 0, size - 1 do
                ...
                local px, py = x + i, y + j
                if grid[px][py] and grid[px][py] ~= 9999 and grid[px][py] ~= 9998 then
                    grid[px][py] = nil
                end
            end
        end
